...
//쉬다 올게~
    ...
    Thread.Sleep(0); // 조건부 양보 => 나보다 우선순위가 낮은 애들한테는 양보 불가 => 우선순위가 나보다 같거나 높은 쓰레드 있으면 다시 본인한테
    Thread.Yield(); // 관대한 양보 => 관대하게 양보할테니, 지금 실행이 가능한 쓰레드가 있으면 실행하세요 => 실행 가능한 애가 없으면 남은 시간 소진
```
사실 쓰레드를 왔다갔다 하는 과정은 생각보다 많은 자원을 소비한다. 즉 차라리 스핀락을 사용하는 것이 더 효율적일 수도 있다. 물론 스핀락 예제처럼 간단한 코드가 아닌, 좀 헤비한 작업이라면, 와일문 안에 이렇게 ContextSwitching 코드를 넣어주면 도움이 될 수도 있다.
